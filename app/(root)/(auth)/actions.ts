...
export async function loginAction(
  _previousState: AuthActionState = emptyState,
  formData: FormData,
...
export async function signupAction(
  ...
  const emailRedirectTo = origin
    ? `${new URL(origin).origin}/auth/callback?next=${encodeURIComponent(next)}`
    : undefined;
  const supabase = await createClient();
  ...
  return { success: "Account created. Check your email and follow the confirmation link to continue." };
}
