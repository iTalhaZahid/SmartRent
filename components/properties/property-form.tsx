...
import { createPropertyAction } from "@/app/(root)/properties/new/actions";
import type { PropertyActionState } from "@/app/(root)/properties/new/actions";
...
export type PropertyFormValues = {
  title: string; description: string; address: string; city: string; propertyType: string;
  bedrooms: number; bathrooms: number; maxOccupants: number;
  rentalMode: (typeof rentalModes)[number]["value"];
  shortTermPrice: number | null; monthlyPrice: number | null;
  ...
  return (
    <form action={formAction} className="space-y-6" noValidate>
      ...
      <FormSection number="01" title="Property details" description="Help renters quickly understand what makes your place a good fit.">
        ...
        <Field label="Property type" name="propertyType" error={errorFor("propertyType")}>
          <select id="propertyType" name="propertyType" defaultValue={initial?.propertyType ?? ""} className={selectClass}>
            ...
            {['Apartment', 'House', 'Condo', 'Studio', 'Townhouse', 'Room'].map((type) => <option key={type}>{type}</option>)}
          </select>
        </Field>
          ...
          <div className="grid gap-3 sm:grid-cols-3">
            {rentalModes.map((mode) => (
              <label key={mode.value} className={cn("relative cursor-pointer rounded-xl border p-4 transition-colors has-focus-visible:ring-2 has-focus-visible:ring-primary", rentalMode === mode.value ? "border-teal-700 bg-teal-50" : "border-slate-200 bg-white hover:border-slate-300")}>
                ...
                <span className="flex items-center justify-between gap-2"><span className="text-sm font-semibold text-slate-900">{mode.label}</span>{rentalMode === mode.value && <Check className="size-4 text-teal-700" />}</span>
      ...
      <FormSection number="05" title="Property photos" description={initial ? "Add new photos to the existing gallery." : "Add up to 6 clear photos. The first photo becomes the listing cover."}>
        <div className="sm:col-span-2">
          <label htmlFor="photos" className="flex min-h-36 cursor-pointer flex-col items-center justify-center rounded-xl border border-dashed border-slate-300 bg-slate-50 px-5 text-center transition-colors hover:border-teal-600 hover:bg-teal-50/50 has-focus-visible:ring-2 has-focus-visible:ring-primary">
            ...
            <input id="photos" name="photos" type="file" accept="image/jpeg,image/png,image/webp" multiple className="sr-only" onChange={(event) => {
              photos.forEach((photo) => URL.revokeObjectURL(photo.url));
              setPhotos(Array.from(event.target.files ?? []).map((file) => ({ name: file.name, url: URL.createObjectURL(file) })));
            }} />
          </label>
          {photos.length > 6 && <p role="alert" className="mt-2 text-sm text-red-600">Choose no more than 6 photos.</p>}
          {!!photos.length && <div className="mt-4 grid grid-cols-2 gap-3 sm:grid-cols-3">{photos.slice(0, 6).map((photo, index) => <div key={photo.url} className="relative aspect-[4/3] overflow-hidden rounded-lg bg-slate-100"><Image src={photo.url} alt={`Preview of ${photo.name}`} fill unoptimized className="object-cover" />{index === 0 && <span className="absolute left-2 top-2 rounded bg-slate-950/75 px-2 py-1 text-[10px] font-bold uppercase tracking-wide text-white">Cover</span>}<span className="absolute inset-x-0 bottom-0 truncate bg-slate-950/65 px-2 py-1 text-[10px] text-white">{photo.name}</span></div>)}</div>}
        </div>
      </FormSection>
      ...
      <div className="flex flex-col-reverse gap-3 border-t border-slate-200 pt-6 sm:flex-row sm:items-center sm:justify-between">
        ...
        {initial ? <ConfirmSubmitButton title="Update listing?" message="Your changes will replace the current property details shown to renters." disabled={pending} className="h-11 px-6">
          {pending ? <LoaderCircle className="animate-spin" /> : <Building2 />}
          {pending ? "Saving..." : "Save changes"}
          {!pending && <ArrowRight />}
        </ConfirmSubmitButton> : <Button type="submit" disabled={pending} className="h-11 px-6">
          {pending ? <LoaderCircle className="animate-spin" /> : <Building2 />}
  ...
  return (
    <section className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm sm:p-7">
      <div className="mb-6 flex gap-4 border-b border-slate-100 pb-5">
        ...
        <div><h2 className="font-semibold text-slate-950">{title}</h2><p className="mt-1 text-sm text-slate-500">{description}</p></div>
      </div>
...
function Field({ label, name, error, hint, className, children }: { label: string; name: string; error?: string; hint?: string; className?: string; children: React.ReactNode }) {
  ...
  return (
    <div className={cn("space-y-2", className)}>
      ...
      <div aria-describedby={describedBy} aria-invalid={!!error}>{children}</div>
      {error ? <p id={`${name}-error`} className="text-sm text-red-600">{error}</p> : hint ? <p id={`${name}-hint`} className="text-xs text-slate-500">{hint}</p> : null}
    </div>
  );
}

function PriceInput({ id, name, suffix, defaultValue }: { id: string; name: string; suffix: string; defaultValue?: number | null }) {
  return <div className="relative"><span className="pointer-events-none absolute inset-y-0 left-3 flex items-center text-sm font-medium text-slate-500">PKR</span><Input id={id} name={name} type="number" inputMode="numeric" min="1" step="1" defaultValue={defaultValue ?? undefined} placeholder="0" onKeyDown={(event) => { if ([".", ",", "e", "E", "+", "-"].includes(event.key)) event.preventDefault(); }} onInput={(event) => { event.currentTarget.value = event.currentTarget.value.replace(/\D/g, ""); }} className="pl-12 pr-20" /><span className="pointer-events-none absolute inset-y-0 right-3 flex items-center text-xs text-slate-400">{suffix}</span></div>;
}
