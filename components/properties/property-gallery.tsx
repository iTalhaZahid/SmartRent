    ...
    <section aria-label="Property photo gallery" className="grid grid-cols-[auto_minmax(0,1fr)_auto] items-center gap-2 sm:gap-4">
      ...
      <div className="min-w-0 overflow-hidden rounded-2xl bg-slate-100 shadow-sm">
        <Swiper
          ...
          className="aspect-[16/10] sm:aspect-[16/7] [&_.swiper-pagination-bullet-active]:bg-white"
        >
          {images.map((image, index) => (
            <SwiperSlide key={image.src}>
              <div className="relative size-full">
                ...
                <span className="absolute bottom-4 right-4 rounded-full bg-slate-950/65 px-3 py-1 text-xs font-medium text-white backdrop-blur-sm">{index + 1} / {images.length}</span>
              </div>
            </SwiperSlide>
          ))}
        </Swiper>
