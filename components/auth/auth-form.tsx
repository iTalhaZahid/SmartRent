    ...
    <form action={formAction} className="min-w-0 space-y-5" noValidate>
      ...
      {state.success && (
        <div role="status" className="flex gap-3 rounded-lg border border-green-200 bg-green-50 px-4 py-3 text-sm leading-5 text-green-800">
          <CheckCircle2 className="mt-0.5 size-4 shrink-0" aria-hidden="true" /><span>{state.success}</span>
        </div>
      )}
      ...
      {isSignup && (
        <div className="space-y-2">
          ...
          {errorFor("fullName") && <p id="fullName-error" className="text-sm text-red-600">{errorFor("fullName")}</p>}
        </div>
      )}
      ...
      <div className="space-y-2">
        ...
        {errorFor("email") && <p id="email-error" className="text-sm text-red-600">{errorFor("email")}</p>}
      </div>
      ...
      <div className="space-y-2">
        ...
        {errorFor("password") ? <p id="password-error" className="text-sm text-red-600">{errorFor("password")}</p> : isSignup ? <p id="password-hint" className="text-xs text-slate-500">Use 8+ characters with uppercase, lowercase, and a number.</p> : null}
      </div>
      ...
      <p className="border-t border-slate-200 pt-5 text-center text-sm text-slate-600">
        ...
        <Link href={`${isSignup ? "/login" : "/register"}${next === "/" ? "" : `?next=${encodeURIComponent(next)}`}`} className="font-semibold text-primary hover:text-teal-800 hover:underline">{isSignup ? "Sign in" : "Create an account"}</Link>
      </p>
    </form>
  );
}
...
function RoleOption({ name, value, checked, icon: Icon, title, subtitle, onChange }: { name: string; value: "RENTER" | "OWNER"; checked: boolean; icon: typeof UserRound; title: string; subtitle: string; onChange: (value: "RENTER" | "OWNER") => void }) {
  return (
    <label className={cn("relative flex min-h-20 min-w-0 cursor-pointer items-center gap-3 rounded-lg border bg-white p-3 text-left transition-all has-focus-visible:outline-none has-focus-visible:ring-2 has-focus-visible:ring-primary has-focus-visible:ring-offset-2", checked ? "border-primary bg-teal-50 shadow-[0_0_0_1px_#0f766e]" : "border-slate-200 hover:border-slate-300 hover:bg-slate-50")}>
      ...
      <span className={cn("grid size-9 shrink-0 place-items-center rounded-lg", checked ? "bg-teal-100 text-teal-800" : "bg-slate-100 text-slate-600")}><Icon className="size-4" /></span>
      <span className="min-w-0"><span className="block text-sm font-semibold text-slate-900">{title}</span><span className="mt-0.5 block text-xs text-slate-500">{subtitle}</span></span>
    </label>
  );
}
